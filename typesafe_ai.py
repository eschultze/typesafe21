...
API_URL = os.environ["TYPESAFE_API_URL"]
...
def _api_request(payload: dict) -> dict:
    """Send request to TypeSafe API with exponential backoff retries."""
...
def get_ai_decision(
    ...
    # Dealer bust probability by upcard (approximate, from basic strategy literature)
    dealer_bust_pct = {
    ...
    # Choice criteria: use not_for for disambiguation, add other option
    criteria = {
        "hit": {
...
def _score_to_bet(score: float, min_bet: int, balance: int, true_count: int = 0) -> int:
    """Convert Score response to bet using balance-based scaling.
    
    The AI decides aggressiveness via Score (0-4).
    Bet is a percentage of balance, scaled by the score.
    """
    # Bet as percentage of balance, based on AI's score
